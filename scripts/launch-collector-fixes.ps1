...
foreach ($t in $Terminals) {
    ...
    Write-Host "Launching Terminal $t ($name)..." -ForegroundColor Green
    Start-Process powershell -ArgumentList "-NoExit", "-Command", "Set-Location '$ProjectDir'; claude --dangerously-skip-permissions '$prompt'"
}

Write-Host ""
Write-Host "All terminals launched." -ForegroundColor Cyan
Write-Host "Each terminal validates with: npm run orch:collector:bench"
Write-Host "Terminal 4 does the final integration sweep."
